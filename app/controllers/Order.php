...
//    $this->userModel = $this->model('User');
      ...
      if (isLoggedIn()) {
        if ($this->products->shopModel->search(getUsername(), $id)) {
          if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $data = ['username' => getUsername(), 'productId' => $id, 'item' => $this->products->itemModel->getProductById($id), 'payments' => $this->products->shopModel->getPaymentsForUsername(getUsername()), 'quantity' => $_POST['quantity'], 'payment' => $_POST['payment'], 'payment_err' => ''];
            ...
            if (empty($data['payment_err'])) {
              ...
              $data2 = ['username' => getUsername(), 'productId' => $id, 'newQuantity' => $data['item']->quantity - $_POST['quantity'], 'quantity' => $_POST['quantity'], 'visaNumber' => $visaNumber, 'visaMoney' => $visaMoney];
            ...
            $data = ['username' => getUsername(), 'item' => $this->products->itemModel->getProductById($id), 'payments' => $this->products->shopModel->getPaymentsForUsername(getUsername())];
            $this->view('users/buy', $data);
          }
        } else {
  ...
  public function allOrders() {
    if (isLoggedIn()) {
      $data = ['products' => $this->users->userModel->getOrdersForUser(getUsername()), 'seller' => $this->products->itemModel->getOrdersForSeller(getUsername())];
