...
<h1 class="text-center mt-4 mb-5" style="color: #333">
  <a style="font-size: 12px" class="btn btn-danger confirm" href="<?php echo URLROOT ?>/products/delete/<?php echo $data['id'] ?>"><i class="fas fa-trash-alt"></i> Delete</a>
  Edit Product
</h1> 
<form style="width: 40%" class="mx-auto" method="POST" action="<?php echo URLROOT ?>/products/edit/<?php echo $data['id'] ?>" enctype="multipart/form-data">
  ...
  <div class="form-group">
    ...
    <textarea class="form-control <?php echo (!empty($data['features_err']))? 'is-invalid' : '' ?>" id="feature" rows="8" name="feature"><?php echo $data['features'] ?></textarea>
  ...
  <div class="custom-file mt-2 mb-3">
    ...
    <small id="emailHelp" class="form-text text-muted">If you want to change the image select new one otherwise leave it empty</small>
  </div>
  <div class="custom-control custom-radio mt-3">
    <input type="radio" id="rent" name="renting" class="custom-control-input" value="1" <?php echo ($data['rentStatus'] == 1)? 'checked' : '' ?>>
    <label class="custom-control-label" for="rent">The Product is for renting</label>
  </div>
  <div class="custom-control custom-radio">
    <input type="radio" id="norent" name="renting" class="custom-control-input" value="0" <?php echo ($data['rentStatus'] == 0)? 'checked' : '' ?>>
  ...
  <div class="custom-control custom-radio mt-3">
    <input type="radio" id="new" name="isNew" class="custom-control-input" value="1" <?php echo ($data['isNew'] == 1)? 'checked' : '' ?>>
    <label class="custom-control-label" for="new">The Product is New</label>
  </div>
  <div class="custom-control custom-radio">
    <input type="radio" id="used" name="isNew" class="custom-control-input" value="0" <?php echo ($data['isNew'] == 0)? 'checked' : '' ?>>
    <label class="custom-control-label" for="used">The Product is Used</label>
  </div>
  <button type="submit" class="btn btn-primary customBtn btn-block mt-3" style="border-radius: 5px;padding: 8px 0;">Save Changes</button>
</form>
